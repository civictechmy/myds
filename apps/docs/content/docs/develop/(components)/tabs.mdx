...
import { TabsContentExample, TabsListExample } from "@/components/tabs-example";
import { ArrowForwardIcon } from "@civictechmy/myds-react/icon";
...
The primary container that composes all tab components together, managing state and interactions between triggers and their content panels. It is being built on top of `TabsPrimitive.Root`.
...
A container that houses and aligns all tab triggers, providing a consistent layout for navigation elements. It is being built on top of `TabsPrimitive.List`.
...
Interactive element that toggles visibility of its corresponding tab panel when selected. It is being built on top of `TabsPrimitive.Trigger`.

### TabsContent

The panel that displays content when its corresponding trigger is selected, hiding when another tab is activated. It is being built on top of `TabsPrimitive.Content`.

### TabsIcon

The `TabsIcon` is a wrapper component that applies consistent styling and sizing to icons within `TabsTrigger`, using context-based size variants (`small` or `medium`).

### TabsCounter

The `TabsCounter` is a component that displays numerical indicators or badges within `TabTrigger`, adjusting its text size based on the parent tabs' context (`small` or `medium`).

<small>
  For more in-depth customization, you can refer to API references in this
  [documentation](https://www.radix-ui.com/primitives/docs/components/tabs#api-reference)
</small>
